...
def search_paralogs(logger, protein_fasta, m8_outfile, genome_max_scores_path, scores_statistics_dir, error_file_path,
    ...
    while not os.path.exists(m8_outfile):
        ...
        cmd = f'mmseqs easy-search {protein_fasta} {protein_fasta} {m8_outfile} {tmp_dir} --format-output {consts.MMSEQS_OUTPUT_FORMAT} --min-seq-id {identity_cutoff} -c {coverage_cutoff} --cov-mode 0 -e {e_value_cutoff} --comp-bias-corr 0 --threads 1 -v 1'
