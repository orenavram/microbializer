...
# MODIFIED, 6/20/2023, Gianna Durante This code has been modified to ignore incomplete codons and
# to use .5 in place of a codon with no counts in a RSCU table
...
class CodonAdaptationIndex:
    ...
    def generate_index_from_record(self, record):
        """Generate a codon usage index from a FASTA file of CDS sequences.
        ...
        # count codon occurrences in the file.
        self._count_codons_from_record(record)
    ...
    def _count_codons_from_record(self, record):
        ...
        #From updated class
        for codon in self.index:
            if self.index[codon] == 0:
                self.index[codon] = .5
    ...
    def cai_for_gene(self, dna_sequence):
        ...
        # be used.
        if self.index == {}:
            print("Error, no index was generated")
        ...
        for i in range(0, len(dna_sequence), 3):
            ...
            if codon in self.index:
                ...
                if codon not in ["ATG", "TGG"]:
                    #print(codon)
                    #print(self.index[codon])
                    try:
                        cai_value += math.log(self.index[codon])
                        cai_length += 1
                    except:
                        cai_value += math.log(.5)
                        cai_length += 1
            # some indices may not include stop codons:
            elif codon not in ["TGA", "TAA", "TAG"]:
                #MODIFIED: Code below is commented out to ignore incomplete codons rather than rasing an error
                #raise TypeError(f"illegal codon in sequence: {codon}.\n{self.index}")
                continue
            
        #print("val", cai_value)
        #print("len", cai_length)
            ...
            for record in SeqIO.parse(handle, "fasta"):
                ...
                for i in range(0, len(sequence), 3):
                    ...
                    try:
                        ...
                        continue
                    #MODIFIED: Code below is modified to ignore incomplete codons rather than rasing an error
                       # raise ValueError(
